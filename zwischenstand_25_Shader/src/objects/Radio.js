class Radio extends THREE.Group {
    ...
    addParts() {
        ...
        var metallMaterial = new THREE.MeshStandardMaterial({color: 0xE7E7E7, roughness: 0.2, metalness: 0.2});

        var korpusGeometry = new THREE.BoxGeometry(30, 20, 8);
        var korpusMaterial = new THREE.MeshLambertMaterial({
            color: 0xE77C3E
        });
        var path = "../../lib/three.js-r113/examples/textures/cube/Bridge2/";
        ...
        antenneGeometry.applyMatrix4(new THREE.Matrix4().makeTranslation(0, 12.5, 0));
        ...
        einschalterAnimation.setSpeed(2.0);
        einschalter.userData = einschalterAnimation;
        this.animations.push(einschalterAnimation);

        // Lautsprecherteil Lamellen Shader
        // --------------------------------
        var lautsprecherteilGeometry = new THREE.BoxGeometry(28, 11.5, 1);
        var lamellen_uniforms = {
            amount: {type: "f", value: 60.0},
            color1: {type: "c", value: new THREE.Color(0x999999)},
            color2: {type: "c", value: new THREE.Color(0xEEEEEE)},
        };
        var material_Lamellen = new THREE.ShaderMaterial({
            uniforms: lamellen_uniforms,
            vertexShader: document.getElementById('lamellen.vert').textContent,
            fragmentShader: document.getElementById('lamellen.frag').textContent
        });
        var lautsprecherteil = new THREE.Mesh(lautsprecherteilGeometry, material_Lamellen);
